...
void USQ_add_point(UncommittedStroke* uncommitted_stroke_ptr, StrokePoint point) {
  ...
  if (uncommitted_stroke.stroke.points_len == uncommitted_stroke.points_capacity) {
    StrokePoint* res = realloc(
        uncommitted_stroke.points, uncommitted_stroke.points_capacity * 2 * sizeof(StrokePoint)
    );
    if (res != NULL) {
  ...
  uncommitted_stroke_ptr->points[uncommitted_stroke.stroke.points_len] = point;
  uncommitted_stroke_ptr->stroke.points_len = uncommitted_stroke.stroke.points_len + 1;
}

void USQ_destroy(UncommittedStrokesQueue* q) {
  pthread_mutex_destroy(&q->mutex);
  pthread_cond_destroy(&q->cond);
  // free(q->array); This needs to also free all the uncomitted stroke pointers
